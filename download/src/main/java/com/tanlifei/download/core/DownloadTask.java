...
import com.tanlifei.download.core.ConnectThread.ConnectListener;
import com.tanlifei.download.core.DownloadThread.DownloadListener;
import com.tanlifei.download.entity.DownloadStatusLevel;
import com.tanlifei.download.entity.test.DownloadEntry;
import com.tanlifei.download.utils.FileUtils;
...
*/
		/*if(entry.getRanges() == null){
			List<RangesEntry> ranges = new ArrayList<RangesEntry>();
			RangesEntry rangesEntry ;
			for(int i = 0; i < DownloadConfig.getInstance().getMax_download_threads(); i++){
				rangesEntry = new RangesEntry();
				rangesEntry.setRange_key(i);
				rangesEntry.setRangeTotalLength(0);
				rangesEntry.setUrl(entry.getUrl());
				ranges.add(rangesEntry);
			}
		...
		if(flag){
			...
			Trace.d("DownloadTask==>" + "startMultiThreadDownload#####" +entry.getName() + " is already done");
		}
	}
